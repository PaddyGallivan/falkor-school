...
const PROJECT_NAME = 'School';           // e.g. "KBT", "School", "FamilyComps"
const PROJECT_CONTEXT = 'PE teacher at Williamstown Primary School — lessons, sport programs, admin, student management';   // e.g. "Kow Brainer Trivia business"
...
- Paddy teaches PE at Williamstown Primary School, Melbourne
- Responsible for: weekly lesson plans, sport programs, excursions, equipment, school events
- Key admin: permission slips, parent comms, report writing, timetabling
- Sport programs: school sport, interschool competitions, athletics carnival, swimming carnival
- Student ages: primary school (Prep–Year 6)
- Term-based calendar — 4 terms per year

Proactively flag: upcoming carnivals/events needing planning, overdue permission slips, equipment that needs ordering.
...
<title>Falkor — ${PROJECT_NAME}</title>
<style>
...
export default {
  async fetch(request, env) {
    ...
    const _pu = new URL(request.url);
    if (_pu.pathname === "/ping") return new Response(JSON.stringify({ok:true,worker:"falkor-school",ts:new Date().toISOString()}), {headers:{"Content-Type":"application/json","Access-Control-Allow-Origin":"*"}});
